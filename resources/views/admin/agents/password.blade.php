...
>
    <x-page-title title="Réinitialiser le mot de passe de l’agent" subtitle="Compte: {{ $agent->email }}" />

    <form method="POST" action="{{ route('admin.agents.password.update', $agent) }}" class="max-w-3xl space-y-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        @csrf
        @method('PATCH')
