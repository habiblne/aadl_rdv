...
>
    <x-page-title title="Réinitialiser le mot de passe de l’administrateur" subtitle="Compte: {{ $admin->email }}" />

    <form method="POST" action="{{ route('admin.admins.password.update', $admin) }}" class="max-w-3xl space-y-5 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
        @csrf
        @method('PATCH')
